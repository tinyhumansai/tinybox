...
fn process() -> ProcessId {
    ProcessId::new("p1-0").expect("a valid process id")
}

#[test]
fn a_minted_id_is_valid_and_distinct() {
    let first = mint();
    let second = mint();
    ...
    assert!(ProcessId::new(first.as_str()).is_ok());
}

#[test]
fn the_pid_file_lives_outside_the_workspace() {
...
#[test]
fn starting_runs_through_a_shell_because_backgrounding_needs_one() {
    let started = start(&process(), &ExecRequest::new(["sleep", "60"])).expect("a command");
...
#[test]
fn the_pid_is_recorded_before_the_wrapper_exits() {
    ...
    assert!(line.ends_with(&format!("'{PID_DIR}/tinybox-p1-0.pid'")), "{line:?}");
}

#[test]
fn output_is_discarded_so_a_full_pipe_cannot_block_the_process() {
    let started = start(&process(), &ExecRequest::new(["server"])).expect("a command");

    assert!(started.argv[2].contains("</dev/null >/dev/null 2>&1"));
}

#[test]
fn the_command_is_quoted_so_a_filename_cannot_inject() {
    let started = start(
        &process(),
        &ExecRequest::new(["echo", "; rm -rf /"]),
    )
    .expect("a command");

    // One quoted word, so the semicolon is data.
    assert!(started.argv[2].contains(r"'echo' '; rm -rf /'"), "{:?}", started.argv[2]);
}
...
fn the_working_directory_and_environment_reach_the_backgrounded_command() {
    ...
    let started = start(&process(), &request).expect("a command");
...
#[test]
fn a_caller_payload_does_not_reach_the_wrapper() {
...
#[test]
fn an_empty_command_is_refused_here_rather_than_by_a_backend() {
    let error = start(&process(), &ExecRequest::new(Vec::<String>::new())).unwrap_err();
...
#[test]
fn stopping_escalates_and_always_clears_the_pid_file() {
...
#[test]
fn a_sub_second_grace_still_waits_a_whole_second() {
...
#[test]
fn a_started_process_is_reported_running_and_then_stops() {
    ...
    let id = mint();
    let started = start(&id, &ExecRequest::new(["sleep", "30"])).expect("a command");

    let Some(_) = run(&started) else {
