...
fn the_same_tree_hashes_the_same_way_twice() -> Result<()> {
    ...
    assert_eq!(
        Fingerprint::of_directory(dir.path(), &[])?,
        Fingerprint::of_directory(dir.path(), &[])?
    );
    Ok(())
}
...
fn two_trees_with_identical_content_agree() -> Result<()> {
    ...
    assert_eq!(
        Fingerprint::of_directory(one.path(), &[])?,
        Fingerprint::of_directory(other.path(), &[])?
    );
    Ok(())
}
...
fn changing_a_file_changes_the_fingerprint() -> Result<()> {
    let dir = tree()?;
    let before = Fingerprint::of_directory(dir.path(), &[])?;

    write(dir.path(), "a.txt", "changed")?;

    assert_ne!(Fingerprint::of_directory(dir.path(), &[])?, before);
    Ok(())
}
...
fn adding_or_removing_a_file_changes_the_fingerprint() -> Result<()> {
    ...
    let with_extra = Fingerprint::of_directory(dir.path(), &[])?;
    assert_ne!(with_extra, before);

    fs::remove_file(dir.path().join("c.txt")).map_err(|error| Error::io("remove", &error))?;
    assert_eq!(Fingerprint::of_directory(dir.path(), &[])?, before);
    Ok(())
}
...
fn an_excluded_directory_does_not_affect_the_fingerprint() -> Result<()> {
    let dir = tree()?;
    let before = Fingerprint::of_directory(dir.path(), &[".git".to_owned()])?;

    write(dir.path(), ".git/HEAD", "ref: refs/heads/main")?;
    write(dir.path(), "nested/.git/HEAD", "ref: refs/heads/other")?;

    // The name matches anywhere in the tree, not just at the root.
    assert_eq!(
        Fingerprint::of_directory(dir.path(), &[".git".to_owned()])?,
        before
    );
...
fn a_link_cycle_does_not_hang_the_walk() -> Result<()> {
    ...
    assert!(Fingerprint::of_directory(dir.path(), &[]).is_ok());
    Ok(())
}
...
fn an_unreadable_root_is_reported() {
    assert!(matches!(
        Fingerprint::of_directory("/tinybox-no-such-directory", &[]),
        Err(Error::Io { .. })
    ));
}
...
fn a_fingerprint_round_trips_through_text() -> Result<()> {
    let dir = tree()?;
    let fingerprint = Fingerprint::of_directory(dir.path(), &[])?;
